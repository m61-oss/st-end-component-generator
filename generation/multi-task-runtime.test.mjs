...
const schemes = () => ({
  ...
  taskSchemes: [{ id: 'task-prompt-1', name: 'Task prompt', snapshot: { taskPrompt: 'saved task prompt' } }],
...
test('task runtime resolves every selected scheme and keeps current component content', () => {
  ...
  assert.equal(runtime.taskPrompt, 'saved task prompt');
  assert.equal(runtime.taskSchemeId, 'task-prompt-1');
...
test('API, task instruction, and component schemes are required and stale ids fail before queue dispatch', () => {
  assert.throws(
    () => resolveMultiTaskRuntimeSettings(base(), { id: 'a', taskSchemeId: 'task-prompt-1', componentSchemeId: 'component-1' }, schemes()),
    (error) => error.code === 'missing-api-scheme',
  );
  assert.throws(
    () => resolveMultiTaskRuntimeSettings(base(), { id: 'a', apiSchemeId: 'api-1', taskSchemeId: 'task-prompt-1' }, schemes()),
    (error) => error.code === 'missing-component-scheme',
  );
  assert.throws(
    () => resolveMultiTaskRuntimeSettings(base(), { id: 'a', apiSchemeId: 'api-1', componentSchemeId: 'component-1' }, schemes()),
    (error) => error.code === 'missing-task-scheme',
  );
  assert.throws(
    () => resolveMultiTaskRuntimeSettings(base(), { id: 'a', apiSchemeId: 'stale', taskSchemeId: 'task-prompt-1', componentSchemeId: 'component-1' }, schemes()),
    (error) => error.code === 'unknown-api-scheme',
  );
  assert.throws(
    () => resolveMultiTaskRuntimeSettings(base(), { id: 'a', apiSchemeId: 'api-1', taskSchemeId: 'stale', componentSchemeId: 'component-1' }, schemes()),
    (error) => error.code === 'unknown-task-scheme',
  );
  assert.throws(
    () => resolveMultiTaskRuntimeSettings(base(), { id: 'a', apiSchemeId: 'api-1', taskSchemeId: 'task-prompt-1', componentSchemeId: 'component-1', presetSchemeId: 'stale' }, schemes()),
