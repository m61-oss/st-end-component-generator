...
assert.match(source, /import \{[\s\S]*?buildApiRequestParts,[\s\S]*?parseApiAdditionalParameters,[\s\S]*?parseApiNumericSettings,[\s\S]*?serializeRequestHeadersYaml,[\s\S]*?\} from '\.\/api\/api-request-parameters\.js\?ver=0\.1\.6';/, 'request parameter helpers should be imported');
