...
test('floor panel retry regenerates only failed multi-task items', () => {
  ...
  assert.match(actionSource, /action === 'generate'[\s\S]*generateMultiTasks\(batchTaskIds\)/);
});

test('floor panel total generation includes every batch-enabled task after a partial run', () => {
  const actionSource = indexSource.slice(
    indexSource.indexOf('async function runMessageFloorPanelAction'),
    indexSource.indexOf('function bindMessageFloorPanel'),
  );

  assert.match(actionSource, /const batchTaskIds = allTasks\.filter\(\(task\) => task\.batchEnabled !== false\)\.map\(\(task\) => task\.id\)/);
  assert.match(actionSource, /action === 'generate'[\s\S]*generateMultiTasks\(batchTaskIds\)/);
